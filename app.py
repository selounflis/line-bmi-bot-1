 from flask import Flak, request, abort
...
def handle_image_message(event):
    ...
    normalized_image_array = (image_array.astype.astype(np.float32) / 127.0) - 1
    data = np.ndarray(sahpe=(1, 224, 224, 3), dtype=np.float32)
